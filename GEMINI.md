...
3.  **모임(Groups) 기능 구현**:
    *   백엔드 `Group`, `GroupMember` 엔티티 및 API(`POST /api/groups`, `GET /api/groups`) 구현 완료.
    *   프론트엔드 `GroupCreateModal` 및 모임 목록 실시간 연동 완료.
    *   **모임 상세 화면(`groups/[id]`) 및 그룹 캘린더 구현**: 모임 정보 조회, 모임별 메모 목록 및 **모임 전용 일정(캘린더) 연동** 완료. 탭 UI를 통해 메모와 캘린더를 전환하여 확인 가능.
...
6.  **친구 관리(Friends) 기능 구현**:
    *   백엔드 `Friend` 엔티티 및 상태 기반 API(`GET /api/friends`, `POST /api/friends/request`, `PUT /api/friends/accept/{id}`, `DELETE /api/friends/{id}`) 구현 완료.
    *   프론트엔드 친구 관리 페이지 실시간 데이터 연동 및 요청 수락/거절 기능 구현 완료.
    *   **친구 추가 모달(`FriendSearchModal`)**: 이메일을 통한 사용자 검색 및 친구 요청 보내기 기능 통합 완료.

### 🏃 다음 세션에서 바로 시작할 작업
- [ ] **모임 멤버 초대**: 친구 목록을 기반으로 모임에 멤버를 초대하는 기능 추가.
- [ ] **AI 인사이트**: 백엔드 실제 OCR 및 요약 로직 구현.
- [ ] **알림(Notifications) 시스템**: 친구 요청 및 수락 시 실시간 알림 피드 구축.
