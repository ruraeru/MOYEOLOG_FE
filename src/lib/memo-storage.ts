...
const SEED_MEMOS: Omit<Memo, 'id' | 'authorId' | 'createdAt' | 'updatedAt'>[] = [
  {
    title: 'React 훅 정리 노트',
    content:
...
export function getInsight(userId: string, memoId: string): MemoAiInsight | null {
  const insights = readJson<Record<string, MemoAiInsight>>(INSIGHTS_KEY(userId), {});
